...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex min-h-screen bg-[#0f172a]" dir="rtl">
      ...
      {/* Main content */}
      <main className={`flex-1 transition-all duration-300 ${collapsed ? "mr-16" : "mr-64"}`}>
        {/* Top bar */}
        <header className="sticky top-0 z-30 flex h-16 items-center justify-between border-b border-[#334155]/40 bg-[#0f172a]/80 px-6 backdrop-blur-xl">
          <div className="flex items-center gap-3">
            <h2 className="text-sm font-medium text-[#f1f5f9]">
              {navItems.find((i) => pathname === i.href || (i.href !== "/admin" && pathname.startsWith(i.href)))?.label || "الإدارة"}
            </h2>
          </div>
          <div className="flex items-center gap-3">
            ...
            <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-[#1e293b] ring-1 ring-[#d4a843]/20">
              <img src="/ERB.png" alt="" className="h-5 w-5 object-contain" />
            </div>
          </div>
        </header>
